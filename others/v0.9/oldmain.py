...
#  下面描述的每一行代码都有很好的注释，例如：代码的目的，它的功能，
#  在某些情况下返回等：该解决方案的替代方案，其他来源如包含的 PDF 文档也具有代码。
#  按钮的 CSS 样式表在父文件夹的 CSS.txt 中单独给出
#  #
###########################################################################################
...
#  CHANGES.
# 注意 由于大部分工作是在 QT DESIGNER 中完成的，因此您可能看不到此处的样式表，
# 请参阅 Documentation.pdf 此外，许多设置已在 QT DESIGNER 本身中预定义，
# 因此在此函数中会发生什么在这之后当用户更改输入状态时，此处仅删除，因此如果您想修改该文件，
# 请在 QT 设计器中打开相应的 .ui 文件并进行修改，然后返回此处为更改添加功能。
#  #
###########################################################################################
import os.path
import sys
...
#FOR REST OF THE WIDGET GOTO THE Documentation.pdf AND CHECK THE LAYOUT FOR MORE DETAILS.
    ...
    def evaluate(self):
        ...
        origin_list = []
        origin_list.append(list(map(int,self.ui.plainTextEdit_6.toPlainText().split())))
        origin_list.append(list(map(int, self.ui.plainTextEdit_9.toPlainText().split())))
        origin_list.append(list(map(int, self.ui.plainTextEdit_7.toPlainText().split())))
        origin_list.append(list(map(int, self.ui.plainTextEdit_8.toPlainText().split())))
        origin_list.append(list(map(int, self.ui.plainTextEdit_10.toPlainText().split())))
        mark = 0
        for r1 , r2 in zip(origin_list,self.whs):
            ...
            mark+=(tp1/tp2)/5
        mark = mark*100
        self.dialogexec("评分结果","您的测距结果与AI测距结果的匹配得分为：%f分"%mark,icon=r"icons/1x/errorAsset 55.png",btn1="好",btn2="好")
        print(mark)
        pass
